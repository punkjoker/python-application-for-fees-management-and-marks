...
class Registration:
    def __init__(self, root, ls, frame_old2):
        ...
        combo_class = ttk.Combobox(Manage_Frame, width=15, textvariable=self.Email_var, font=("times new roman", 12),
                                   state="readonly")
        combo_class['values'] = ("PlayGroup", "PP1", "PP2")
        combo_class.grid(row=3, column=1, pady=10, padx=20, sticky="w")
        ...
        lbl_Address = Label(Manage_Frame, text="PARENT",bg="LightBlue", fg="RED",
                            font=("times new roman", 20, "bold"))
        ...
        Addbtn = Button(btn_frame, text="NEW STUDENT",bg='black',fg='white', width=10, command=self.add_data).grid(row=0, column=0, padx=10, pady=10)
        ...
        self.Student_table.heading("Address", text="PARENT NAME")
    ...
    def add_data(self):
        # Define the current term (update as necessary)
        current_term = "Term_1_2025"  # Example: Term_1_2025
        student_fees_table = f"student_fees_{current_term}"  # Create dynamic table name

        # Connect to the database
        conn = sqlite3.connect('employee.db')
        c = conn.cursor()

        # Insert data into the main student_data table
        ...
        # Insert data into the term-specific student fees table with initial fee setup
        c.execute(f"INSERT INTO {student_fees_table} (Roll_No, fees_paid, fees_rem) VALUES (?, ?, ?)", (
            self.Roll_No_var.get(),
            0,  # Initial fee amount
            3500  # Initial balance or unpaid amount
        ))

        # Insert data into student_attendance table (if needed)
        c.execute("INSERT INTO student_attendance VALUES (?, ?, ?)", (
            self.Roll_No_var.get(),
            '0',  # Example initial attendance count
            '0'   # Example initial absence count
        ))

        # Insert data into student_marks table (if it exists)
        c.execute("INSERT INTO student_marks (Roll_No) VALUES (?)", (
            self.Roll_No_var.get(),
        ))

        # Commit changes, fetch updated data, clear inputs, and close the connection
        conn.commit()
        self.fetch()
        self.clear()
        conn.close()
