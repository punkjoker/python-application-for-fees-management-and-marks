...
class ClassFees:
    ...
    def print_fee_balances(self):
        ...
        if file_path:
            with open(file_path, 'w') as file:
                ...
                file.write(f"{'Roll No':<20} {'Name':<35} {'Class':<35} {'Fees Paid':<35} {'Fees Due':<20}\n")
                file.write("=" * 150 + "\n")

                for row in balances:
                    file.write(f"{row[0]:<20} {row[1]:<35} {row[2]:<35} {row[3]:<20} {row[4]:<20}\n")
    ...
    def set_new_term(self):
        ...
        try:
            # Step 1: Create a new table for the new term with fees_rem set to the base amount (e.g., 3500)
            cur.execute(f"""
            ...
            cur.execute(f"""
                INSERT OR REPLACE INTO {new_term_table} (Roll_No, fees_paid, fees_rem)
                SELECT Roll_No, 0, fees_rem + 3500  -- Add new term's fee to any remaining balance
